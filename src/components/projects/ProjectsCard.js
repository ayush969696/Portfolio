...
const ProjectsCard = ({ title, githublink, des, src, techStack, date }) => {
  return (
    <div className="w-full p-4 xl:px-8 h-auto xl:py-10 rounded-lg shadow-shadowOne flex flex-col bg-gradient-to-r from-bodyColor to-[#202327] group hover:bg-gradient-to-b hover:from-gray-900 hover:gray-900 transition-colors duration-1000">
      <div className="w-full h-[80%] overflow-hidden rounded-lg">
        <img
          className="w-full h-72 object-cover group-hover:scale-110 duration-300 cursor-pointer"
          src={src}
          alt="src"
        />
      </div>
      <div className="w-full mt-5 flex flex-col gap-6">
        <div>
          ...
          <p className="text-sm text-gray-400 tracking-wide">{date}</p>
          <div className="flex flex-wrap gap-2 mt-3">
            {techStack.map((tech, index) => (
              <span
                key={index}
                className="text-sm border border-yellow-400 text-gray-200 px-2 py-1 rounded"
              >
                {tech}
              </span>
            ))}
